...
window.LANG_en = {
    ...
    actor_hint: 'Enter a name, pick the sender ID to transmit from and the EEP, then send the teach-in telegram so the device learns this gateway. (the actor has to be in teach-in mode first) (the receiver has to be in teach-in mode first) Start teach-in, then trigger the device you want to add: press its teach-in button (or simply use a regular switch, e.g. an F6 rocker - it does not need a teach-in button). The device is added automatically. Devices using a 4BS or UTE learn telegram are recognised with their EEP; bi-directional devices are acknowledged.',    bidir_hint: 'Start teach-in, then trigger the device. On receipt we answer with a teach-in telegram from the next free sender ID and pre-fill the details below.',
